...
export function BookCard({ book, size = "default" }: { book: Book; size?: "default" | "compact" }) {
  return (
    <Link
      href={`/book/${book.slug}`}
      className={`group block shrink-0 ${size === "compact" ? "w-32 sm:w-36" : "w-36 sm:w-44"}`}
    >
      <div className="relative aspect-2/3 overflow-hidden rounded-lg border border-hairline bg-surface shadow-sm transition group-hover:-translate-y-1 group-hover:shadow-lg">
