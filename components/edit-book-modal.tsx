...
export function EditBookModal({
  ...
  const [status, setStatus] = useState<EditableBookStatus>(
    // A "removed" book can transiently reach this modal (e.g. a stale link)
    // before it's purged — there's no dropdown option for that state, so we
    // fall back to "draft" rather than widening the dropdown to match.
  ...
  async function handleSave() {
    ...
    try {
      ...
      onSaved({
        ...
        tags: data.book.tags,
        coverUrl, // cover already saved server-side the moment it was uploaded
      });
      onClose();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg rounded-2xl border border-hairline bg-surface p-6 shadow-xl">
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-full bg-accent px-4 py-2 font-sans text-sm font-semibold text-accent-ink transition disabled:opacity-40 hover:opacity-90"
          >
