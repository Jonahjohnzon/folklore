...
export type BadgeRole = "icon" | "chip" | "card";

export interface BadgeProps {
  category: BadgeCategory;
  tier: number; // 1-5
  name: string;
  description?: string; // e.g. "500 chapters" — shown in "chip" and "card" roles
  role?: BadgeRole;
  size?: number; // pixel height of the icon; ignored/overridden per-role below
}

function BadgeArt({ category, tier, size, title }: { category: BadgeCategory; tier: number; size: number; title: string }) {
  if (category === "reading_milestone") {
    const c = SHIELD_COLORS[tier] ?? SHIELD_COLORS[1];
    return (
      <svg width={size} height={size * (100 / 90)} viewBox="0 0 90 100" role="img" aria-label={title} className="shrink-0">
        <title>{title}</title>
  ...
  return (
    <svg width={size * (70 / 90)} height={size} viewBox="0 0 70 90" role="img" aria-label={title} className="shrink-0">
      <title>{title}</title>
...
/**
 * Drop-anywhere badge display. `role` controls presentation:
 *  - "icon": bare SVG, nothing else — for tight spaces (comment rows, avatars).
 *  - "chip": icon + name in a pill — for inline lists (profile header, badge shelf).
 *  - "card": icon + name + description in a bordered block — for grids (admin, badge-earned modal).
 */
export function Badge({ category, tier, name, description, role = "chip", size }: BadgeProps) {
  if (role === "icon") {
    return <BadgeArt category={category} tier={tier} size={size ?? 28} title={name} />;
  }

  if (role === "chip") {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-full border border-hairline py-1 pl-1 pr-3">
        <BadgeArt category={category} tier={tier} size={size ?? 22} title={name} />
        <span className="font-sans text-xs font-medium text-ink">{name}</span>
      </span>
    );
  }
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-hairline p-3">
      <BadgeArt category={category} tier={tier} size={size ?? 44} title={name} />
      <div>
